...
export default function Footer() {
    return (
        <footer className="flex flex-wrap justify-center md:justify-between overflow-hidden gap-10 md:gap-20 mt-40 py-6 px-6 md:px-16 lg:px-24 xl:px-32 text-[13px] text-gray-500">
            ...
            >
                <Link to='/'>
                    <img className="size-8 aspect-square" src="/favicon.svg" alt="footer logo" width={100} height={100} />
                </Link>
                    ...
                    <div key={index}>
                        ...
                        <ul className="mt-2 space-y-2">
                            {section.links.map((link, index) => (
                                <li key={index}>
                                    <Link to={link.href} className="hover:text-cyan-600 transition">
                                        {link.name}
                                    </Link>
                                </li>
                            ))}
            ...
            >
                ...
                <p className="mt-3 text-center">&copy; {new Date().getFullYear()} <a href="">Thumb Lab</a></p>
            </motion.div>
        </footer>
    );
}
